...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const moodSub = supabase
      ...
      .on("postgres_changes", {
        ...
        showToast(`${partnerProfile?.display_name} is feeling ${mood.label} ${mood.emoji}`);
      })
      .subscribe();
    ...
    const hugSub = supabase
      ...
      .on("postgres_changes", {
        ...
        supabase.from("hugs").delete().eq("id", payload.new.id).then(() => {});
      })
      .subscribe();
    ...
    const unmatchSub = supabase
      ...
      .on("postgres_changes", {
        ...
        if (updated.unmatch_requested && !unmatchPending) {
          showToast(`${partnerProfile?.display_name} wants to unmatch — check the Connect tab`);
        }
      })
      .subscribe();
    ...
    if (profile.partner_id) {
      ...
      fetch("/api/notify", {
        ...
        body: JSON.stringify({
          ...
          body: note.trim() ? `"${note.trim()}"` : "Check in on their vibe ✨",
        }),
      }).catch(() => {});
    }
  };
      ...
      <main className="min-h-screen bg-ink flex flex-col items-center justify-center relative overflow-hidden">
        ...
        <div className="relative flex items-center justify-center mb-8">
          <motion.div
            ...
            style={{ width: 160, height: 160, background: "radial-gradient(circle, rgba(192,132,252,0.15), transparent 70%)" }}
            animate={{ scale: [1, 1.5, 1], opacity: [0.3, 0.8, 0.3] }}
            transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
          />
          <motion.div
            className="absolute rounded-full"
            style={{ width: 100, height: 100, background: "radial-gradient(circle, rgba(192,132,252,0.2), transparent 70%)" }}
  ...
  const theirMood = partnerLatestMood ? getMood(partnerLatestMood.mood as MoodKey) : null;
    ...
    <main className="min-h-screen bg-ink pb-32 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-sm mx-auto px-5 pt-14">
        ...
        <div className="flex items-center justify-between mb-10">
          <div>
            <p className="text-white/30 text-xs font-semibold tracking-widest uppercase">Hey</p>
            <h1 className="font-display text-2xl font-bold text-white">
              {profile?.display_name?.split(" ")[0]} 👋
            </h1>
          </div>
          <button onClick={handleSignOut} className="text-white/20 text-xs hover:text-white/40 transition-colors">
            Sign out
          </button>
        </div>
        ...
        <section className="mb-10">
          <p className="text-white/25 text-[10px] font-bold tracking-widest uppercase mb-5">Your Vibe</p>
          <div className="flex flex-col items-center">
            {myMood ? (
              ...
              >
                ...
                {myLatestMood?.note && (
                  <p className="text-sm italic text-center mt-2" style={{ color: myMood.color + "bb" }}>
                    "{myLatestMood.note}"
                  </p>
                )}
                <p className="text-white/20 text-xs mt-1">
                  {formatDistanceToNow(new Date(myLatestMood!.created_at), { addSuffix: true })}
                </p>
              </motion.div>
            ) : (
              <div className="py-10 text-center">
                <p className="text-white/25 text-sm">Tap below to set your vibe ✨</p>
              </div>
            )}
        ...
        {profile?.partner_id ? (
          <section className="mb-6">
            <p className="text-white/25 text-[10px] font-bold tracking-widest uppercase mb-5">
              {partnerProfile?.display_name ? `${partnerProfile.display_name}'s Vibe` : "Their Vibe"}
            </p>
            ...
            <div className="glass rounded-3xl p-6 flex flex-col items-center gap-3">
              {theirMood ? (
                ...
                >
                  ...
                  {partnerLatestMood?.note && (
                    <p className="text-sm italic text-center" style={{ color: theirMood.color + "bb" }}>
                      "{partnerLatestMood.note}"
                    </p>
                  )}
                  <p className="text-white/20 text-xs">
                    {formatDistanceToNow(new Date(partnerLatestMood!.created_at), { addSuffix: true })}
                  </p>
                </motion.div>
              ) : (
                <p className="text-white/25 text-sm py-4">
                  {partnerProfile?.display_name || "They"} hasn't set their vibe yet
                </p>
              )}
              ...
              >
                {hugSent ? "Hug sent! 💌" : `Send ${partnerProfile?.display_name || "them"} a hug 🫂`}
              </motion.button>
            </div>
          </section>
        ) : (
          <section>
            <div className="glass rounded-3xl p-6 text-center">
              <p className="text-white/30 text-xs font-bold tracking-widest uppercase mb-3">Your Code</p>
              <p className="font-display text-4xl font-bold text-purple-400 tracking-widest mb-2">
                {profile?.partner_code}
              </p>
              <p className="text-white/30 text-sm">Share with your partner on the Connect tab ♡</p>
            </div>
          </section>
        )}
      </div>
              ...
              <div className="p-6 pb-10">
                ...
                <div className="flex justify-center my-6 h-28">
                  <AnimatePresence mode="wait">
                    {chosenMood ? (
                      <motion.div key={chosenMood.key} initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}>
                        <MoodOrb mood={chosenMood} size={90} showLabel={false} />
                      </motion.div>
                    ) : (
                      <motion.p key="placeholder" className="text-5xl opacity-20 self-center">✨</motion.p>
                    )}
                  </AnimatePresence>
                </div>
                  ...
                  {MOODS.map((mood) => {
                    ...
                    return (
                      <motion.button
                        ...
                        style={{
                          backgroundColor: isSelected ? mood.color + "22" : "rgba(255,255,255,0.03)",
                          borderColor: isSelected ? mood.color + "66" : "rgba(255,255,255,0.06)",
                        }}
                      >
                        <span className="text-2xl">{mood.emoji}</span>
                        <span className="text-[10px] font-semibold" style={{ color: isSelected ? mood.color : "rgba(255,255,255,0.4)" }}>
                          {mood.label}
                        </span>
                      </motion.button>
                    );
                ...
                <div className="flex gap-3 mt-2">
                  ...
                  <motion.button
                    ...
                    style={{
                      ...
                      boxShadow: chosenMood ? `0 8px 32px ${chosenMood.color}55` : "none",
                    }}
                  >
                    {saving ? "Saving..." : selectedMood ? `OK — I'm ${getMood(selectedMood).label} ${getMood(selectedMood).emoji}` : "Pick a mood first"}
                  </motion.button>
                </div>
              </div>
            </motion.div>
        ...
        {incomingHug && (
          ...
          >
            <div className="glass rounded-3xl p-8 text-center border border-pink-500/20 shadow-2xl max-w-xs w-full">
              ...
              >🫂</motion.div>
              <h3 className="font-display text-xl font-bold mb-1">You got a hug!</h3>
              <p className="text-white/50 text-sm mb-6">{incomingHug} is thinking of you ♡</p>
              <button
